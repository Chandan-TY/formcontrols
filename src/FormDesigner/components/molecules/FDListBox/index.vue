...
<script lang="ts">
import { Component, Prop, Model, Emit, Mixins, Watch } from 'vue-property-decorator'
...
export default class FDListBox extends Mixins(FdControlVue) {
  ...
  handleMultiSelect (e: MouseEvent) {
    ...
    // const tempPath = ((<Array<HTMLTableElement>>e.path))
    const tempPath = e.composedPath()
    // console.log('Temp', tempPath )
    // e.composedPath()
    targetElement.focus()
    console.log('e.target', targetElement.innerText[0])
  ...
  clearOptionBGColorAndChecked (e: MouseEvent | KeyboardEvent) {
    const tempPath = e.composedPath()
    for (let index = 0; index < tempPath.length; index++) {
      const element = tempPath[index] as HTMLTableElement
      if (element.className === 'table-body') {
