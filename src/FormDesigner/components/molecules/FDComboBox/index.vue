  ...
  <div class="custom-select" :tabindex="tabindex" :style="customSelectObj"
   :title="properties.ControlTipText">
    <div class="combobox" :style="boxStyleObj">
      <!-- <div> -->
      <input type="text" :value="this.selectionData[0]" :style="inputStyleObj" @focus="isVisible=true" @blur="isVisible=false" />
      ...
      <div class="items" :class="{ selectHide: !open }" v-if="open">
        ...
        <div  class="item" style="height:20px;border-bottom:1px solid black;" v-if="computedOptions.length===0"></div>
        <div
          class="item"
          ...
          :style="{
            borderBottom:
              i === computedOptions.length - 1 ? '1px solid black' : '',
          }"
        >
        <span>
            <input
              ...
              class="item"
              :name="i"
              @mousedown.left="handleMouseClick($event, option)"
              @mouseover="handleOptionInput($event, option)"
              @mouseleave="handleMouseLeave($event)"
            />
        </span>
          <div style="display:inline">
            <span style="padding-left:12px">{{ option }}</span>
          </div>
      </div> -->
